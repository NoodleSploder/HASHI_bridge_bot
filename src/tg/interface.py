...
from src.tg.controller import Controller
from src.tg.keyboards import main_menu_keyboard, create_notice_keyboard
import logging
...
def eth(update, context):
    text = controller.eth(update)
    context.bot.send_message(chat_id=update.effective_chat.id, text=text, parse_mode='Markdown',
                             reply_markup=main_menu_keyboard)


def dai(update, context):
    text = controller.dai(update)
    context.bot.send_message(chat_id=update.effective_chat.id, text=text, parse_mode='Markdown')

def pswap(update, context):
    text = controller.pswap(update)
    context.bot.send_message(chat_id=update.effective_chat.id, text=text, parse_mode='Markdown')

def xor(update, context):
    text = controller.xor(update)
    context.bot.send_message(chat_id=update.effective_chat.id, text=text, parse_mode='Markdown')

def val(update, context):
    text = controller.val(update)
    context.bot.send_message(chat_id=update.effective_chat.id, text=text, parse_mode='Markdown')

def ceres(update, context):
    text = controller.ceres(update)
    context.bot.send_message(chat_id=update.effective_chat.id, text=text, parse_mode='Markdown')

def usdc(update, context):
    text = controller.usdc(update)
    context.bot.send_message(chat_id=update.effective_chat.id, text=text, parse_mode='Markdown')
...
def create_notice_command(update, context):
    text, keyboard = 'Choose tx speed (fast is highly recommended):', create_notice_keyboard
    context.bot.send_message(chat_id=update.effective_chat.id, text=text, reply_markup=keyboard)
...
def create_conversation_handlers():
    ...
    get_notices_conversation_handler = ConversationHandler(
        entry_points=[CommandHandler('my_notices', get_notices)],
        states={
...
def create_handlers():
    ...
    handlers = [CommandHandler('start', start), CommandHandler('help', help), CommandHandler('info', info),
                CommandHandler('donate', donate), CommandHandler('pswap', pswap), CommandHandler('ticker', ticker),
                CommandHandler('eth', eth), CommandHandler('dai', dai), CommandHandler('xor', xor),
                CommandHandler('ceres', ceres), CommandHandler('usdc', usdc), CommandHandler('val', val),
